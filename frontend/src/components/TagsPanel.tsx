 ...
 * butonu vardir. Butona basilinca o etiketi tasiyan columns (schema.tablo.kolon seklinde)
 * aciklanir — TableSidebar'daki schema acilir/kapanir mantigiyla ayni: birden fazla tag'in
 ...
 */
export function TagsPanel({ tags, onLoadUsage, onRename, onDelete }: TagsPanelProps) {
  const { t } = useTranslation();
  const { canWrite } = useAuth();
  const confirm = useConfirm();
  // TableSidebar'daki schema acilir/kapanir deseniyle ayni (bkz. expandedSchemaIds): Set
  ...
  const [usageByTagId, setUsageByTagId] = useState<Map<number, ColumnUsage[]>>(new Map());
                ...
                {isEditing ? (
                  ...
                  <>
                    {/* Detay artik ayri bir "Ayrıntı" butonuyla degil, isme (genel olarak
                        satirin bu alanina) tiklayinca aciliyor/kapaniyor — bkz. TableSidebar'daki
