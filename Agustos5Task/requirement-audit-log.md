...
2026-08-06'da implemente edildi (backend/week3): Faz 1-4 tamamlandı — `AuditLog` entity/repository,
`AuditLogService` (fail-closed), `TabloService`/`SchemaService`/`TagService`/`KullaniciService`'teki
tüm mutasyon noktalarına audit çağrısı, `GET /api/audit-loglar` (ADMIN, filtreli/sayfalı), testler.
Yol boyunca bulunan/çözülen sorunlar için bkz. `DECISIONS.md` "Kalıcı audit log" bölümü.
