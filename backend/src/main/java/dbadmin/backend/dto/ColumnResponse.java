...
public record ColumnResponse(
        ...
        @Schema(description = "Bu kolonun gercek tablonun PRIMARY KEY'ine dahil olup olmadigi (bkz. "
                        + "CreateColumnRequest.primaryKey aciklamasi).", example = "false")
                boolean primaryKey) {

    public static ColumnResponse from(DataColumn column) {
        Tag tag = column.getTag();
        Long tagId = tag == null ? null : tag.getId();
        String tagName = tag == null ? null : tag.getName();
        return new ColumnResponse(
                column.getId(), column.getName(), column.getType(), tagId, tagName, column.isPrimaryKey());
    }
}
